...
# Function to infer app name from the port
def get_app_name():
...
def build_app_container_mapping():
    ...
    logger.info("Total log lines: %d", len(lines))
    for line in lines:
        logger.debug("Processing log line: %s", line.strip())
        ...
        if match:
            ...
            logger.debug("Line did not match expected format: %s", line.strip())
    
    logger.info("Final app_to_containers mapping: %s", app_to_containers)
    return app_to_containers
...
def filter_logs(container_ids):
    ...
    if not container_ids:
        logger.warning("No container IDs provided for filtering logs. Returning all logs.")
        return all_logs
    ...
    for log in all_logs:
        for container_id in container_ids:
            if f"Handled by: {container_id}" in log or (f" | Status: " in log and container_id in all_logs[all_logs.index(log) - 1]):
                ...
                break
    logger.info("Filtered %d logs for container IDs: %s", len(filtered_logs), container_ids)
...
def parse_logs(container_ids):
    ...
    if not os.path.exists(log_file):
        logger.warning("Log file does not exist for parsing.")
        return events
    ...
    logger.info("Parsed %d events for container IDs: %s", len(events), container_ids)
    return events
...
def cluster_event(event_type, collection_id):
    ...
    if request.method == 'GET':
        ...
        log = log_request(path, status=status)
    else:  # POST request
        try:
...
def get_metrics():
    ...
    logger.info("Server filter: %s, Container IDs: %s", server_filter, container_ids)
    if server_filter != 'all' and not container_ids:
        logger.warning("No container IDs found for server %s. Returning empty metrics.", server_filter)
        return jsonify({
            'total_requests': 0,
            'success_rate': 0,
            'avg_cpu': 0,
...
def get_event_distribution():
    ...
    logger.info("Server filter: %s, Container IDs: %s", server_filter, container_ids)
    if server_filter != 'all' and not container_ids:
        logger.warning("No container IDs found for server %s. Returning empty event distribution.", server_filter)
        return jsonify({'FAIL': 0, 'SCHEDULE': 0, 'FINISH': 0, 'ENABLE': 0, 'EVICT': 0, 'LOST': 0})
...
def get_server_load():
    ...
    logger.info("Server filter: %s, Container IDs: %s", server_filter, container_ids)
    if server_filter != 'all' and not container_ids:
        logger.warning("No container IDs found for server %s. Returning empty server load.", server_filter)
        return jsonify({'app1': 0, 'app2': 0, 'app3': 0})
...
def get_cpu_memory_usage():
    ...
    logger.info("Server filter: %s, Container IDs: %s", server_filter, container_ids)
    if server_filter != 'all' and not container_ids:
        logger.warning("No container IDs found for server %s. Returning empty CPU/memory usage.", server_filter)
        return jsonify({'cpu_usage': [0], 'memory_usage': [0]})
...
def get_recent_traces():
    ...
    logger.info("Server filter: %s, Container IDs: %s", server_filter, container_ids)
    if server_filter != 'all' and not container_ids:
        logger.warning("No container IDs found for server %s. Returning empty traces.", server_filter)
        return jsonify([])
...
def get_logs():
    ...
    logger.info("Server filter: %s, Container IDs: %s", server_filter, container_ids)
    if server_filter != 'all' and not container_ids:
        logger.warning("No container IDs found for server %s. Returning empty logs.", server_filter)
        return jsonify([])
